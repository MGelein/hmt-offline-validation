class Paleo{
  ...
  String manuscript;
  //The loaded paleo lines
  String[] lines;
  
  //Creates a new paleo analysis for this repository
  Paleo(String manuscript, String target){
    this.target = target;
  ...
  **/
  void load(){
    //The baseFile Url
    String fileName = "paleography/paleo";
  ...
  **/
  void validate(){
    ...
    for(int i = 1; i < lines.length; i++){
      ...
      if(imgUrn.valid){
        reportLine += td(getImageFromUrn(imgUrn, 50));
      }else{
    ...
    r.save("paleo" + MANUSCRIPT + (TARGET.equals(SCHOLIA) ? "_scholia" : "") + ".html");
    r.open();
